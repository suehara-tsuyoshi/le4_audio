...
def calculate_score():
  ...
  technique_score = int(interval_correct_rate*(shakuri_count+vibrato_count)/(2*now_playing_sec))
...
frame2.pack(side=tkinter.TOP, padx = (0, 5), pady=(25, 0))

frame3 = tkinter.Frame(root, bd = 1, relief = SOLID, width = 590, height = 580, background="white")
...
lyrics_frame = tkinter.LabelFrame(master=frame1, text="歌詞", font=("Arial", 25), width=800, height=120, background="white")
lyrics_frame.pack(side=TOP)
lyrics_frame.propagate(False)
lyrics_label = tkinter.Label(master=lyrics_frame, text="", font=("", 30), background="white")
lyrics_label.pack(side=TOP)
...
def input_callback(in_data, frame_count, time_info, status_flags):
  ...
  if len(x_stacked_data) >= FRAME_SIZE:
    ...
    # interval_correct_rate = 0.99 * interval_correct_rate + 0.01 * cos_similarity(x_f0, lemon_data[idx_guide])
    interval_correct_rate = (interval_correct_rate * idx_guide + cos_similarity(x_f0, lemon_data[idx_guide]))/(idx_guide+1)
